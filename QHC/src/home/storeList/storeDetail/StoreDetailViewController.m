...
-(id)initWithTitle:(NSString*)title andStoreId:(NSString*)index {
    self = [super init];
    if (self) {
        self.title = title;
        self.storeId = index;
...
- (void)viewDidLoad {
    ...
    StoreDetailView *storeDetailView = [[StoreDetailView alloc] initWithFrame:CGRectMake(0.0, titleView.frame.size.height, self.view.frame.size.width, self.view.frame.size.height - titleView.frame.size.height) andStoreName:self.title storeID:storeId];
    [self.view addSubview:storeDetailView];
...
-(void)collectThisStore {
    ...
    if (userId && userId.length > 0) {
        ...
        [param setObject:self.storeId forKey:@"id"];
