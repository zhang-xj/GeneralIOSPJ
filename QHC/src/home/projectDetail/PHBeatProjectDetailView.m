...
@implementation PHBeatProjectDetailView
...
-(void)initTableView {
    self.contentTableView = [[UITableView alloc] initWithFrame:CGRectMake(0.0, 0.0, self.frame.size.width, self.frame.size.height) style:UITableViewStyleGrouped];
...
-(void)touchAction:(id)sender{
    //    if ([@"青花敲术" isEqualToString:projectName]) {
    //        QHCQinghuaBeatPicView *picView = [[QHCQinghuaBeatPicView alloc]initWithFrame:CGRectMake(0.0, 0.0, self.frame.size.width, self.frame.size.height)];
...
-(void)getProjectDetailData:(NSString*)proId {
    ...
    //创建异步请求
    NSString *urlStr = @"Project/QHQSDetails.aspx";
    ...
    [httpRequest sendHttpRequestByPost:urlStr params:nil];
}
//获取养生顾问可预约时间列表
...
- (CGFloat)tableView:(UITableView*)tableView heightForRowAtIndexPath: (NSIndexPath *) indexPath{
    if (indexPath.section == 0) {
    ...
    } else if (indexPath.section == 3) {//青花敲术版权图片只有青花敲术有这个东西
        return 210;
        ...
        NSString *text = [contentDataDic objectForKey:@"history"];
        CGSize lSize = [AppDelegate getStringInLabelSize:text andFont:LABEL_DEFAULT_TEXT_FONT andLabelWidth:tableView.frame.size.width - 30];
            return lSize.height + 20 + 150;
    }
    return 38;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell == nil)
    {
        ...
        if (indexPath.section == 0) {
            if ([indexPath indexAtPosition:1] == 0) {
                ...
                //首次体验价格
                UILabel *priceSingle = [[UILabel alloc] initWithFrame:CGRectMake(labelLeftPading, labelName.frame.origin.y + labelName.frame.size.height + 8, tableView.frame.size.width - 2*labelLeftPading, 16)];
                ...
                //套餐推广价
                UILabel *priceMul = [[UILabel alloc] initWithFrame:CGRectMake(labelLeftPading, priceSingle.frame.origin.y + priceSingle.frame.size.height + 5, 145, 15)];
        ...
        if ([indexPath indexAtPosition:1] == 0) {
            ...
            if([indexPath indexAtPosition:1] == 0){
                ...
                //标题
            UILabel *labelTitle = [[UILabel alloc] initWithFrame:CGRectMake(labelLeftPading, labelLeftPading, tableView.frame.size.width - 2*labelLeftPading, 15)];
            labelTitle.font = LABEL_TITLE_TEXT_FONT;
            labelTitle.textColor = LABEL_TITLE_TEXT_COLOR;
            labelTitle.text = @"版权保护第一养生项目";
            [cell addSubview:labelTitle];
                
            UIImageView *copyrightImgView = [[UIImageView alloc] initWithFrame:CGRectMake(labelLeftPading, labelTitle.frame.origin.y + labelTitle.frame.size.height, tableView.frame.size.width - 2*labelLeftPading, 174)];
            [copyrightImgView setTag:subViewTag];
            [cell addSubview:copyrightImgView];
        } else if (indexPath.section == 4) {//项目说明
            if([indexPath indexAtPosition:1] == 0){
                //标题
        ...
        } else if (indexPath.section == 5) {//青花敲术发展史
            UIImageView *imgView = [[UIImageView alloc] initWithFrame:CGRectMake(0.0, 0.0, tableView.frame.size.width, 150)];
            [imgView setTag:subViewTag];
            [cell addSubview:imgView];
            UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(labelLeftPading, imgView.frame.origin.y + imgView.frame.size.height + 5, tableView.frame.size.width - 2*labelLeftPading, 20)];
            [label setTag:subViewTag +1];
            label.font = LABEL_DEFAULT_TEXT_FONT;
            label.textColor = LABEL_DEFAULT_TEXT_COLOR;
            label.numberOfLines = 0;
            [cell addSubview:label];
        } else if (indexPath.section == 6) {//选择门店 养生顾问 预约时间等
    ...
    if (indexPath.section == 0) {
        if ([indexPath indexAtPosition:1] == 0) {
            ...
            [imgview sd_setImageWithURL:[NSURL URLWithString:[contentDataDic objectForKey:@"img"]] placeholderImage:[UIImage imageNamed:@"default_detail_img.png"]];
        } else if([indexPath indexAtPosition:1] == 1) {
            //项目名称
            ...
            priceSingle.text = [NSString stringWithFormat:@"首次体验价：¥%.0f元/次", ((NSString*)[contentDataDic objectForKey:@"experienceprice"]).floatValue];
            //套餐推广价格
    ...
    } else if (indexPath.section == 1) {//项目说明
        if ([indexPath indexAtPosition:1] == 0) {
            //项目介绍
        ...
        UIImageView *copyrightImgView = (UIImageView*)[cell viewWithTag:subViewTag];
        [copyrightImgView sd_setImageWithURL:[NSURL URLWithString:[self.contentDataDic objectForKey:@"copyrightimg"]] placeholderImage:[UIImage imageNamed:@"default_detail_img.png"]];
    } else if (indexPath.section == 4) {//项目说明
        if([indexPath indexAtPosition:1] == 1){
            //项目说明
        ...
        UIImageView *imgView = (UIImageView*)[cell viewWithTag:subViewTag];
        [imgView sd_setImageWithURL:[NSURL URLWithString:[self.contentDataDic objectForKey:@"historyimg"]] placeholderImage:[UIImage imageNamed:@"default_detail_img.png"]];
        UILabel *label = (UILabel*)[cell viewWithTag:subViewTag+1];
        NSString *str = [self.contentDataDic objectForKey:@"history"];
